...
def cal_selfsupervised_loss(outs, mode='l1', lambda_drloc=0.0):
    loss = 0.0
    
    if mode == "l1": # l1 regression constraint
        reld_criterion = relative_constraint_l1 
    elif mode == "ce": # cross entropy constraint
        reld_criterion = relative_constraint_ce
    elif mode == "cbr": # cycle-back regression constaint: https://arxiv.org/pdf/1904.07846.pdf
